...
')
[[ "$RUN_STATUS" == "completed-success" ]] || abort_with_error "CI not passed (status=$RUN_STATUS). Aborting."

print_step "$ICON_SUCCESS" "CI passed."

print_step "$ICON_PROCESSING" "Creating pull request..."
PR_OUT=$(gh pr create --title "$TITLE" --body "$BODY_CONTENT" --base "$TARGET_BRANCH" --head "$CURRENT_BRANCH" \
  ${LINKED_ISSUE:+--linked-issue "$LINKED_ISSUE"} 2>&1) || abort_with_error "gh pr create failed: $PR_OUT"
