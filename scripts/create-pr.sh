...
# === CONFIGURATION ===
TARGET_BRANCH="dev"
DEFAULT_METADATA_DIR=".github/issues"
...
print_step "$ICON_SUCCESS" "Parsed metadata from ${METADATA_FILE}"
...
print_step "$ICON_SUCCESS" "All CI checks passed."
...
PR_CREATE_OUTPUT=$(gh pr create \
  ...
  ${LINKED_ISSUE:+--linked-issue "$LINKED_ISSUE"} 2>&1) || {
    echo -e "${RED}${ICON_FAILURE} Failed to create PR. GH CLI output:${NC}"
...
print_step "$ICON_SUCCESS" "Pull Request created: ${PR_URL}"
...
if [[ -n "$MILESTONE" ]]; then
  ...
  if [[ -n "$MILESTONE_ID" ]]; then
    gh api -X PATCH "repos/$REPO/issues/$PR_NUMBER" -f milestone="$MILESTONE_ID" >/dev/null && \
      print_step "$ICON_SUCCESS" "Assigned milestone: $MILESTONE"
  else
    print_step "$ICON_FAILURE" "Milestone '$MILESTONE' not found or assignable."
  fi
fi

echo -e "${GREEN}${ICON_SUCCESS} All done!${NC}"
